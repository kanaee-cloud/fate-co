          ...
          <motion.div 
            ...
            <div>
              {services.map((services, index) => {
                const {name, description, link, url} = services;
                return (
                <div 
                  className='border-b border-[#75BFEC]/70 h-[146px] mb-6 flex'
                  key={index}>
                  <div className='max-w-[476px]'>
                    <h4 className='text-[15px] lg:text-[2rem] text-gradient tracking-wider font-tertiary font-semibold mb-4'>{name}</h4>
                    <p className='text-[10px] font-primary leading-tight'>{description}</p>
                  </div>
